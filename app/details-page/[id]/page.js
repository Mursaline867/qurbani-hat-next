...
export default function DetailsPage({ params }) {
  const { user } = useAuth();
  const animal = animals.find(item => String(item.id) === params.id);
  const handleBooking = e => { e.preventDefault(); e.currentTarget.reset(); toast.success('Booking request submitted successfully!'); };
  if (!animal) return <div className="min-h-screen grid place-items-center"><h1 className="text-3xl font-bold">Animal not found</h1></div>;
